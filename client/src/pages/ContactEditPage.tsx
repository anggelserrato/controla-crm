...
export default function ContactEditPage() {
  ...
  useEffect(() => {
    const fetchContact = async () => {
      try {
        ...
        const message =
          error.response?.data?.message || "Error al cargar el contacto";
        toast.error(message);
        navigate("/contacts");
      } finally {
        setLoading(false);
  ...
  const onSubmit = async (data) => {
    try {
      ...
      const message =
        error.response?.data?.message || "Error al actualizar el contacto";
      toast.error(message);
    }
  };
      ...
      <div className="mx-auto max-w-2xl space-y-6">
        ...
        <div className="rounded-lg border border-border/30 bg-muted/20 p-4 backdrop-blur-sm">
          <p className="text-xs leading-relaxed text-muted-foreground">
            <span className="font-semibold text-foreground">Tip:</span> Los
            cambios se guardarán inmediatamente. Completa los campos requeridos
            (nombre, apellido, email) antes de guardar.
          </p>
        </div>
      </div>
    </div>
